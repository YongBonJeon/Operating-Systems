...
thread_set_nice (int nice) 
{
    ...
    cur->nice = nice;
    cur->priority = (PRI_MAX*Fraction - (cur->recent_cpu / 4) - (cur->nice*Fraction*2)) / Fraction;
...
thread_get_recent_cpu (void) 
{
    ...
    return (100*cur->recent_cpu) / Fraction;
}
...
void update_load_avg_recent_cpu(void)
{
    ...
    load_avg = ((59*load_avg) + (num_ready_threads*Fraction)) / 60;
...
init_thread (struct thread *t, const char *name, int priority)
{
...
#ifdef USERPROG
  /* initialize child running semaphore */
  sema_init(&(t->child_lock), 0);
  /* initialize memory(reaping) semaphore */
  sema_init(&(t->mem_lock), 0);
  /* initialize child list */
  list_init(&(t->child_list));
  /* add thread to child list */
  ...
  /* project 3 */
  t->block_time = 0;
  t->nice = running_thread()->nice;     /* inherit */
