...
struct thread
  {
    ...
    /* project 3 */
    int block_time;         /* block during this time */
    ...
    struct semaphore child_lock;    /* child running semaphore */
    struct semaphore mem_lock;    /* memory(reaping) semaphore */
    struct list child_list;                    /* child list */
    struct list_elem child_elem;       /* child list element */
    int child_exit_status;                  /* child exit status */
    struct file* file_descriptor[128];  /* independent file descriptors */
