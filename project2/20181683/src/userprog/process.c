...
process_execute (const char *file_name) 
{
    ...
    char cmd_name[256]; // executable file name
    int i;
    ...
    /* return tid to process_wait's parameter */
    return tid;
}
  ...
  struct thread* t;
	...
	for(struct list_elem* e = list_begin(&(thread_current()->child_list));
			e != list_end(&(thread_current()->child_list)); e = list_next(e))
      {
        t = list_entry(e, struct thread, child_elem);
        if(child_tid == t->tid){
          /* wait for child process(loading) exit*/
          sema_down(&(t->child_lock));
          /* get child process exit status */
          exit_status = t->child_exit_status;
          list_remove(&(t->child_elem));
          /* parent do reaping, so stop waiting */
          sema_up(&(t->mem_lock));
          return exit_status;
        }
    }
...
process_exit (void)
{
  ...
  if (pd != NULL) 
    ...
    /* talk child process exit to parent */
...
void construct_stack(char* argv[32], int argc, void** esp);
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  char *argv[64];      // parsed argv
  int argc = 0;            // number of argv
  char *next_ptr;
  char *token;

  strlcpy(temp_filename, file_name, strlen(file_name) + 1);
  token = strtok_r(temp_filename, " ", &next_ptr);
  while(token != NULL){  
    argv[argc++] = token;
...
void construct_stack(char **argv, int argc, void **esp){
  ...
  /* Word alignment */
  int argv_length = (int)(PHYS_BASE - *esp);
  int word_alignment = 0;
  if(argv_length % 4 != 0){
    word_alignment = 4 - (argv_length) % 4;
  }
  *esp -= word_alignment;

  /* Push address of argv in stack , argv[argc] ~ argv[0] */
  *esp -= 4;
  **(uint32_t **)esp = 0;
